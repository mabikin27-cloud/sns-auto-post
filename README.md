...
LINE で「ネタ」を送ると、Gemini が Instagram / LINE / ブログ用の投稿文を生成し、スプレッドシートに記録します。結果の確認はスプレッドシートで行い、LINE には開始・完了（またはエラー）のみ通知します。
...
4. 完了したら LINE に「生成が完了しました。スプレッドシートでご確認ください。」を 1 通だけ Push。エラー時はエラー内容を 1 通 Push。
